...
const WorkstationAuthWrapper: React.FC<WorkstationAuthWrapperProps> = ({ isDarkMode }) => {
  ...
  const handleWsnameChange = useCallback((wsname: string, valid: boolean) => {
    console.log('[WorkstationAuthWrapper] WSNAME changed:', wsname, 'valid:', valid);
  ...
  const handleLogin = useCallback(async () => {
    ...
    try {
      // Check if workstation name is available (PostgreSQL session validation)
      const validateResponse = await fetch(`http://localhost:8000/api/v1/sessions/validate/${workstationName}`);
      
      if (validateResponse.status === 409) {
        // Workstation is already active
        const validationData = await validateResponse.json();
        const existingSession = validationData.existing_session;
        throw new Error(`Workstation ${workstationName} is already active (User: ${existingSession?.username || 'unknown'}, Last activity: ${existingSession?.last_activity || 'unknown'})`);
      }
      ...
      const validationData = await validateResponse.json();
      console.log('Workstation validation result:', validationData);
